#!/usr/bin/env python3
"""
Model Architecture Summarizer for Safetensors
--------------------------------------------
This script analyzes safetensors files and generates a condensed summary
of the model architecture including shapes and data types.
"""
import json
import re
import argparse
...
def parse_args():
    ...
    # Removed the redundant --dir argument
    parser.add_argument("--output", "-o", type=str, help="Path to output file (optional)")
    # Removed --include-dtype argument, dtype is now always included
...
# Removed get_dtype_string function as header provides dtype string directly
...
# Removed get_tensor_shape_from_file as its logic is integrated into _get_tensor_metadata
...
# Removed include_dtype parameter from function definition
    ...
    for structure, original_names in grouped_params.items():
        ...
        for condensed_name in condensed_names:
            ...
            if representative_name:
                ...
                # Always include dtype if available (removed include_dtype check)
                if dtype:
                     output_str += f",{dtype}"
...
def main():
    ...
    try:
        ...
        # Call summarize_architecture without include_dtype argument
        condensed_summary = summarize_architecture(args.dir_path, args.verbose)
